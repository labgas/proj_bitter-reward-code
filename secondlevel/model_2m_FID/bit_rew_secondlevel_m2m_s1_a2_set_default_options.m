%% bit_rew_secondlevel_m2m_s1_a2_set_default_options.m
%
%
% USAGE
%
...
subjs2exclude_data = {'sub-010' 'sub-018'};                         % default empty     Subjects to be excluded from data objects, for example because of missing session not allowing all contrasts to be calculated, example {'sub-010' 'sub-018'}


%% PREP_3A_RUN_SECOND_LEVEL_REGRESSION_AND_SAVE
% -------------------------------------------------------------------------

maskname_glm = which('bit_rew_mask_all_regions.nii');  % default use of gray matter mask based on canlab 2023 atlas
...
atlasname_glm = which('bit_rew_mask_all_regions.mat'); % default use of new canlab2023 atlas (coarse 2mm version for now, current CANlab default is fine 1mm and in fsl space)
...
maskname_svm = which('bit_rew_mask_all_regions.nii');           % default use of gray matter mask based on canlab 2023 atlas
                                                                                    % model-specific maskdir defined in a_set_up_paths_always_run_first script
                                                                                    % if you do not want to mask, change to []
                                                                                    % if you want to use a custom mask, put it in maskdir and change name here
                                                                                    % used in this script only for visualization of unthresholded results
...
atlasname_svm = which('bit_rew_mask_all_regions.mat');                  % atlas object used for labeling regions
                                                                            %   Two options
                                                                            %       1. atlas name from load_atlas.m for different atlas than
